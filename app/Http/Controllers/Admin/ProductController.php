...
class ProductController extends BaseController
{
    ...
    public function store(StoreProductFormRequest $request)
    {
       ...
       //To update the price in database with tax rate
        //
        //

        $price = (int)$request->input('price');
        $with_tax_price = $price * 1.22;
        $with_tax_price = (string)$with_tax_price;
        $params["price"] = $with_tax_price;

        $price = (int)$request->input('sale_price');
        $with_tax_price = $price * 1.22;
        $with_tax_price = (string)$with_tax_price;
        $params["sale_price"] = $with_tax_price;
